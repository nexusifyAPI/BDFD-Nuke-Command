# 🔄 Callback ($onInteraction)

## Gatilho:
```
$onInteraction
```

## Código:
```
$nomention
$suppressErrors[❌ Ocorreu um erro ao processar a interação.]

$if[$checkContains[$customID;nuke-]==true]

$if[$checkContains[$customID;-$authorID]==false]
$ephemeral
$sendMessage[❌ Apenas quem executou o comando !nuke pode usar estes botões.]
$stop
$endif

$if[$checkContains[$customID;aceptar]==true]

$jsonParse[$getServerVar[NukeData]]
$var[c_id;$json[canal_id]]
$var[c_nombre;$json[canal_nombre]]
$var[c_tipo;$json[canal_tipo]]
$var[c_posicion;$json[canal_posicion]]
$var[c_categoria;$json[canal_categoria]]
$var[c_topic;$json[canal_topic]]
$var[c_nsfw;$json[canal_nsfw]]
$var[c_slowmode;$json[canal_slowmode]]
$var[c_autor;$json[autor_id]]
$var[c_mensaje;$json[mensaje_id]]

$if[$var[c_id]==]
$ephemeral
$sendMessage[❌ Não há dados de nuke pendentes. Execute !nuke primeiro.]
$stop
$endif

$if[$serverChannelExists[$var[c_id]]==false]
$ephemeral
$sendMessage[❌ O canal original não existe mais. Não é possível completar o nuke.]
$stop
$endif

$if[$checkUserPerms[$botID;managechannels]==false]
$ephemeral
$sendMessage[❌ Não tenho permissão de managechannels para completar o nuke.]
$stop
$endif

$try

$var[nsfw_final;no]
$if[$var[c_nsfw]==true]
$var[nsfw_final;yes]
$endif

$var[topic_final;]
$if[$var[c_topic]!=Sem tópico]
$var[topic_final;$var[c_topic]]
$endif

$deleteChannels[$var[c_id]]

$if[$var[c_categoria]!=none]
$createChannel[$var[c_nombre];$var[c_tipo];$var[c_categoria]]
$else
$createChannel[$var[c_nombre];$var[c_tipo]]
$endif

$var[nuevo_canal;$findChannel[$var[c_nombre]]]

$replyIn[2s]
$modifyChannel[$var[nuevo_canal];!unchanged;$var[topic_final];$var[nsfw_final];$var[c_posicion];!unchanged]

$if[$var[c_slowmode]>0]
$slowmode[$var[nuevo_canal];$var[c_slowmode]s]
$endif

$jsonParse[{}]
$setServerVar[NukeData;$jsonStringify]

$sendMessage[✅ **Nuke completado com sucesso**

💣 Canal antigo excluído e recriado com a mesma configuração:
**Novo canal:** <#$var[nuevo_canal]>
**Nome:** #$var[c_nombre]
**Posição:** $var[c_posicion]
**Categoria:** $if[$var[c_categoria]==none]Nenhuma$else<#$var[c_categoria]>$endif
**NSFW:** $if[$var[c_nsfw]==true]Sim$elseNo$endif
**Slowmode:** $var[c_slowmode]s

Executado por: <@$authorID>]

$catch

$ephemeral
$sendMessage[❌ Ocorreu um erro ao processar o nuke: $error[message]

O canal original NÃO foi excluído. Tente novamente.]
$jsonParse[{}]
$setServerVar[NukeData;$jsonStringify]

$endtry

$stop
$endif

$if[$checkContains[$customID;rechazar]==true]

$jsonParse[{}]
$setServerVar[NukeData;$jsonStringify]

$sendMessage[❌ **Nuke cancelado**

O canal não foi modificado. Você pode executar !nuke novamente quando quiser.]

$stop
$endif

$endif
```
